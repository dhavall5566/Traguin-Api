#!/usr/bin/env python3
"""Generate tamil_nadu_tn_002_005_batch_defs.py from tmp_tn_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_tn_pdfs"
OUT_PATH = _SCRIPTS / "tamil_nadu_tn_002_005_batch_defs.py"
PRICE_NOTE = "On Request (Premium Customised)"
BUILDER_ORDER = [f"TN-{i:03d}" for i in range(2, 6)]
...
def extract_destinations_tamil_nadu(text: str) -> str:
    lines = text.splitlines()
    collecting = False
    parts: list[str] = []
    for i, ln in enumerate(lines):
        stripped = ln.strip()
        if re.match(r"DESTINATIONS(?:\s*COVERED)?:\s*$", stripped, re.I):
            collecting = True
            continue
        if re.match(r"^DESTINATIONS\s*$", stripped, re.I):
            if i + 1 < len(lines) and re.match(r"^COVERED:\s*$", lines[i + 1].strip(), re.I):
                collecting = True
            continue
        if re.match(r"^COVERED:\s*$", stripped, re.I) and not parts:
            collecting = True
            continue
        if collecting:
            if not stripped:
...
def extract_tour_code(text: str) -> str:
    ...
    return m.group(1) if m else "TRG-TN-000"


def extract_title_tamil_nadu(text: str) -> str:
    patterns = [
        r"TRAGUIN PREMIUM TAMIL NADU TOUR\s*\nPACKAGE\s*\n(.+?)(?:\nTRAGUIN Premium|\nWelcome|\nTOUR OVERVIEW)",
        r"TRAGUIN PREMIUM TAMIL NADU TOUR PACKAGE\s*\n(.+?)(?:\nTRAGUIN Premium|\nWelcome|\nTOUR OVERVIEW)",
        r"TRAGUIN PREMIUM TAMIL NADU TOUR\s*\nPACKAGE\s*\n(.+)",
    ]
    for pat in patterns:
        ...
        if m:
            ...
            if len(title) > 12:
                return title
    return "Tamil Nadu Premium Luxury Tour"


def extract_vehicle_meals_tamil_nadu(text: str) -> str:
    flat = re.sub(r"\s+", " ", text)
    for label in (
        "VEHICLE / MEALS:",
        "VEHICLE & MEALS:",
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"tn-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str) -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_tamil_nadu(text)} {extract_title_tamil_nadu(text)}".lower()
    candidates = [
        ("Family", ("family", "group")),
        ("Luxury", ("luxury", "premium", "bespoke")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love", "newlywed")),
        ("Culture", ("culture", "heritage", "temple", "spiritual", "pilgrimage", "divine")),
        ...
        ("Nature", ("nature", "hill", "nilgiri", "ooty", "tea", "lake")),
    ]
    moods: list[str] = []
...
def parse_day_blocks_tamil_nadu(text: str) -> list[dict]:
    start = re.search(
...
def extract_category_tamil_nadu(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
...
def parse_tamil_nadu_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    for item in excluded:
        for part in re.split(r"\s*✘\s*", item):
            ...
            if part and len(part) > 10:
                cleaned_exc.append(part)
    return included, cleaned_exc


def _clean_tamil_nadu_hotel_line(line: str) -> list[str]:
    line = re.sub(r"/\s*similar\w*.*$", "", line, flags=re.I).strip()
    line = re.sub(r"similar\s*(Deluxe|Premium|Luxury)", "", line, flags=re.I).strip()
    line = re.sub(r"(Resort|Hill|Ooty|Coonoor|Hotel)(Premium|Deluxe|Luxury)", r"\1 / \2", line, flags=re.I)
    line = re.sub(r"(Centre)(Welcomhotel)", r"\1 / \2", line, flags=re.I)
    parts: list[str] = []
    for chunk in re.split(r"\s*/\s*", line):
        suffix = re.search(
            r"\s+(Deluxe Valley View|Premium Balcony|Luxury Heritage Garden|VVIP Royal|Executive Suite|"
            r"Custom Tier|Presidential Suite|Private Luxury|Heritage Suite|MAPAI|Room|Suite|Plan|Kit|Tier|Cottage|Villa).*$",
            chunk,
            re.I,
        )
        if suffix and suffix.start() > 0:
            chunk = chunk[: suffix.start()].strip()
        chunk = re.sub(r"\s+", " ", chunk).strip(" /")
        if (
            chunk
            and len(chunk) > 3
            and chunk.lower() not in {"similar", "hotel", "resort", "traguin"}
            and "TRAGUIN" not in chunk.upper()
        ):
            parts.append(chunk)
    return parts


def parse_tamil_nadu_hotel_options(text: str) -> list[dict]:
    """Parse Tamil Nadu PDF table-style hotel option blocks (4 tiers)."""
    section_m = re.search(
    ...
    for m in pattern.finditer(section):
        ...
        props: list[str] = []
        raw_lines: list[str] = []
        for raw_ln in m.group(3).splitlines():
            ln = raw_ln.strip()
            if not ln or any(rx.search(ln) for rx in skip_res):
                continue
            if re.search(r"\b(Curated Cultural|Boutique Spa Access|Garden Views)\b", ln, re.I):
                continue
            if re.fullmatch(
                r"(MAPAI.*|Breakfast.*|Dinner.*|Suite|Room|Plan|Kit|Tier|Cottage|Villa|Inclusion Kit.*|Bespoke Signature.*)",
                ln,
                re.I,
            ):
                continue
        ...
        for ln in raw_lines:
            if merged_lines and (
                len(ln) < 22
                or re.fullmatch(
                    r"(Hotel|Resort|Suite|Island|Spa|Palace|Cottage|Gardenia|Kulam|Trichy|Bay|Mahe|Perumal|Chola)",
                    ln,
                    re.I,
                )
            ):
                merged_lines[-1] = f"{merged_lines[-1]} {ln}"
            else:
                merged_lines.append(ln)
        for ml in merged_lines:
            if "TRAGUIN" in ml.upper():
                continue
            for part in _clean_tamil_nadu_hotel_line(ml):
                if part not in props and "TRAGUIN" not in part.upper():
                    props.append(part)
        if not props:
            continue
        name = " | ".join(props[:4])[:128]
        options.append(
            {
                "sort_order": num,
                "category": category,
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_tamil_nadu(text)
    tour_code = extract_tour_code(raw)
    destinations = extract_destinations_tamil_nadu(text)
    hotels = parse_tamil_nadu_hotel_options(text)
    if len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "Tamil Nadu",
                "nights_label": nights,
    ...
    primary_loc = destinations.split("•")[0].strip() or "Tamil Nadu"
    for hotel in hotels:
    ...
    included, excluded = parse_tamil_nadu_inclusions_exclusions(raw)
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_tamil_nadu(text)
    destinations = meta.get("destinations") or extract_destinations_tamil_nadu(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"State / Country: Tamil Nadu / India | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_tamil_nadu(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_tamil_nadu(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_tamil_nadu_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_tamil_nadu_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_tamil_nadu(text)
    seo_desc = (
        f"Premium {duration} Tamil Nadu package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for TN-002 through TN-005 Tamil Nadu domestic packages."""
...
'''
    ...
    builder_names = [f"build_tn_{serial.split('-')[1]}" for serial in BUILDER_ORDER]
