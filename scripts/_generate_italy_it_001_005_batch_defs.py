#!/usr/bin/env python3
"""Generate italy_it_001_005_batch_defs.py from tmp_it_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_it_pdfs"
OUT_PATH = _SCRIPTS / "italy_it_001_005_batch_defs.py"
PRICE_NOTE = "On Request (Premium Customised)"
BUILDER_ORDER = [f"IT-{i:03d}" for i in range(1, 6)]

TITLE_MAP = {
    "IT-001": "Italy Highlights Family Tour",
    "IT-002": "Romantic Italy Honeymoon",
    "IT-003": "Luxury Italy Grand Expedition",
    "IT-004": "Italy Classical Family Tour",
    "IT-005": "Grand Italy Tour",
}

SLUG_MAP = {
    "IT-001": "it-001-italy-highlights-family-tour",
    "IT-002": "it-002-romantic-italy-honeymoon",
    "IT-003": "it-003-luxury-italy-grand-expedition",
    "IT-004": "it-004-italy-classical-family-tour",
    "IT-005": "it-005-grand-italy-tour",
}

ITIN_SLUG_MAP = {
    "IT-001": "it-001-italy-highlights-itinerary",
    "IT-002": "it-002-romantic-italy-itinerary",
    "IT-003": "it-003-luxury-italy-itinerary",
    "IT-004": "it-004-italy-classical-family-itinerary",
    "IT-005": "it-005-grand-italy-itinerary",
}
...
_IT_FOOTER_LINE = re.compile(
    ...
    r"(?: • IT-\d{3})?(?: • www\.traguin\.in)?(?: Page \d+ of \d+|\s+\d+)?(?:\s*—\s*[^\n]+)?\s*\d*",
    re.IGNORECASE,
)
_IT_FOOTER_FRAGMENT = re.compile(
    r"(?:TRAGUIN )?(?:Premium )?(?:Luxury )?(?:Holidays|Itinerary|Holiday Proposals?|Family Vacations|Honeymoon Experts)"
    r"(?: • IT-\d{3})?(?: • www\.traguin\.in)?[^\n]*?(?=DAY\s*\d+)",
    re.IGNORECASE,
)
...
def normalize_it_raw(raw: str) -> str:
...
def clean_it_raw(raw: str) -> str:
    raw = normalize_it_raw(raw)
    raw = _IT_FOOTER_LINE.sub("\n", raw)
    text = clean_raw(raw)
    ...
    text = _IT_FOOTER_LINE.sub("\n", text)
    text = _IT_FOOTER_FRAGMENT.sub("\n", text)
    text = _normalize_day_headers(text)
    text = re.sub(r"(DAY\s*\d+\s*\|\s*[^\n]+)\n\s*[—–-]\s*", r"\1 | ", text, flags=re.MULTILINE)
    return text


def extract_destinations_it(text: str) -> str:
...
def extract_tour_code(text: str) -> str:
    flat = re.sub(r"\s+", " ", normalize_it_raw(text))
    for pat in (
    ...
    m = re.search(r"Serial Code:\s*IT-\d{3}", flat, re.I)
    if m:
        return f"TRAGUIN-ITA-{m.group(0).split('-')[-1]}"
    return "TRAGUIN-ITA-000"


def extract_title_it(text: str, serial: str = "") -> str:
    ...
    if tagline:
        return re.sub(r"\s+", " ", tagline.group(1).strip())
    return "Italy Premium Luxury Tour"


def extract_vehicle_meals_it(text: str) -> str:
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"it-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str, serial: str = "") -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_it(text)} {extract_title_it(text, serial)}".lower()
    candidates = [
        ("Family", ("family", "group", "multi-generational")),
        ("Luxury", ("luxury", "premium", "bespoke", "ultra")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Culture", ("culture", "heritage", "vatican", "renaissance", "colosseum", "florence", "rome")),
        ("Romantic", ("romantic", "venice", "gondola", "honeymoon", "canals")),
        ("Food & Wine", ("culinary", "tuscan", "pasta", "wine", "gastronomy")),
...
def parse_day_blocks_it(text: str) -> list[dict]:
    start = re.search(
...
def extract_category_it(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
        re.DOTALL | re.IGNORECASE,
...
def _default_it_inclusions() -> tuple[list[str], list[str]]:
    return (
        [
            "Premium handpicked accommodation across Italy",
            "Private luxury airport transfers and dedicated chauffeur-driven MPV",
            "TRAGUIN 24/7 dedicated on-ground concierge support",
        ],
        [
            "International airfare and Schengen visa fees",
...
def parse_short_inclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if included:
        _, excluded = _default_it_inclusions()
        return included, excluded
    return _default_it_inclusions()


def parse_it_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if not included and not excluded:
        return _default_it_inclusions()
    if not included:
        included = _default_it_inclusions()[0]
    if not excluded:
        excluded = _default_it_inclusions()[1]
    return included, excluded


def parse_it_hotel_options(text: str) -> list[dict]:
    """Parse Italy PDF hotel option blocks."""
    section_m = re.search(
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_it(text, serial)
    tour_code = extract_tour_code(normalize_it_raw(raw))
    destinations = extract_destinations_it(text)
    hotels = parse_it_hotel_options(text)
    if len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "Italy",
                "nights_label": nights,
                "room_type": "Deluxe Room",
    ...
    primary_loc = destinations.split("•")[0].strip() or "Italy"
    for hotel in hotels:
    ...
    if serial in {"IT-001", "IT-002"}:
        included, excluded = parse_short_inclusions(text)
    else:
        included, excluded = parse_it_inclusions_exclusions(raw)
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_it(text)
    destinations = meta.get("destinations") or extract_destinations_it(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"Country: Italy, Europe | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_it(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_it(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_it_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_it_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_it(text)
    seo_desc = (
        f"Premium {duration} Italy package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for IT-001 through IT-005 Italy international packages."""
...
'''
    ...
    builder_names = [f"build_it_{serial.split('-')[1]}" for serial in BUILDER_ORDER if (PDF_DIR / f"{serial}.txt").exists()]
