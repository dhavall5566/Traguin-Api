#!/usr/bin/env python3
"""Generate australia_au_005_batch_defs.py from tmp_au_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_au_pdfs"
OUT_PATH = _SCRIPTS / "australia_au_005_batch_defs.py"
PRICE_NOTE = "On Request (Premium Customised)"
BUILDER_ORDER = ["AU-005"]
...
def clean_au_raw(raw: str) -> str:
    text = clean_raw(raw)
    text = re.sub(r"DESTINATIONS\s*\n\s*COVERED:", "DESTINATIONS COVERED:", text, flags=re.I)
    text = re.sub(
        r"TRAGUIN (?:Premium Luxury Itinerary|Premium Holidays|Luxury Holidays|Luxury Proposals?|Luxury Proposal)(?: • AU-\d{3})?(?: — [^\n]+)?\s*\d*",
        "\n",
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(r"TRAGUIN Luxury Holidays • www\.traguin\.in Page \d+ of \d+", "\n", text, flags=re.I)
    text = re.sub(r"^\s+(DAY\s*\d+)", r"\1", text, flags=re.MULTILINE)
    text = re.sub(r"(DAY\s*\d+\s*\|\s*[^\n]+)\n\s*[—–-]\s*", r"\1 | ", text, flags=re.MULTILINE)
    return text


def extract_destinations_au(text: str) -> str:
    ...
    return re.sub(r"\s+", " ", m.group(1).strip()) if m else "Australia"
...
def extract_tour_code(text: str) -> str:
    ...
    return m.group(1) if m else "TRG-AUS-000"


def extract_title_au(text: str) -> str:
    patterns = [
        r"TRAGUIN PREMIUM AUSTRALIA TOUR\s*\nPACKAGE\s*\n(.+?)(?:\n\d+\s+NIGHTS|\nTRAGUIN Premium|\nTRAGUIN Luxury|\nWelcome|\nTOUR OVERVIEW)",
        r"TRAGUIN PREMIUM AUSTRALIA\s*\nTOUR PACKAGE\s*\n(.+?)(?:\nTRAGUIN Premium|\nTRAGUIN Luxury|\nWelcome|\nTOUR OVERVIEW)",
        r"TRAGUIN PREMIUM AUSTRALIA TOUR PACKAGE\s*\n(.+?)(?:\nTRAGUIN Premium|\nTRAGUIN Luxury|\nWelcome|\nTOUR OVERVIEW)",
    ]
    for pat in patterns:
        ...
        if m:
            ...
            if len(title) > 12:
                return title
    return "Australia Premium Luxury Tour"


def extract_vehicle_meals_au(text: str) -> str:
    flat = re.sub(r"\s+", " ", text)
    for label in (
        "VEHICLE / MEALS:",
        "VEHICLE / VESSEL:",
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"au-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str) -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_au(text)} {extract_title_au(text)}".lower()
    candidates = [
        ("Family", ("family", "group")),
        ("Luxury", ("luxury", "premium", "bespoke")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Culture", ("culture", "heritage", "colonial", "opera", "laneway")),
        ("Beach", ("beach", "coast", "surfers", "harbour", "reef")),
        ("Wildlife", ("wildlife", "koala", "kangaroo", "tiger", "zoo")),
...
def parse_day_blocks_au(text: str) -> list[dict]:
    start = re.search(
        r"(?:YOUR BESPOKE DAY-WISE ITINERARY|THE DEFINITIVE DAY-WISE ITINERARY|DETAILED DAY-WISE ITINERARY|DAY-WISE ITINERARY|DAY WISE ITINERARY)",
        text,
        re.IGNORECASE,
    )
    if start:
...
def extract_category_au(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
...
def parse_au_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    cleaned = _jh.PAGE_RE.sub(" ", text)
    cleaned = re.sub(
        r"TRAGUIN (?:Premium Luxury Itinerary|Premium Holidays|Luxury Holidays|Luxury Proposal|Corporate Luxury Proposal)(?: • AU-\d{3})?(?:\s*—\s*[^\n]+)?\s*\d*",
        " ",
        cleaned,
        flags=re.IGNORECASE,
    )
    ...
    included = [
        ...
        if item and "TRAGUIN Premium" not in item and len(item) > 10
    ]
...
def _au005_hotel_matrix() -> list[dict]:
    nights = "10N multi-city"
    tiers = [
        (
            1,
            "Deluxe",
    ...
    return [
        {
            ...
            "category": cat,
            "name": name,
            "location": "Melbourne | Cairns | Gold Coast | Sydney",
            "nights_label": nights,
            "room_type": "Premium Room",
            "meal_plan": "Daily Premium Breakfast & Gourmet Dinner",
            "stars": 5 if "ultra" in cat.lower() else 4,
            "description": f"OPTION {num:02d} – {cat.upper()}: {name}",
        }
        for num, cat, name in tiers
    ]


def parse_au_hotel_options(text: str) -> list[dict]:
    """Parse Australia PDF hotel option blocks (4 tiers, single or multi-city)."""
    if "AU-005" in text or "Melbourne (3N)" in text:
        matrix = _au005_hotel_matrix()
        if matrix:
            return matrix
    section_m = re.search(
        r"HANDPICKED PREMIUM (?:ACCOMMODATION OPTIONS|HOTEL OPTIONS)\s*(.+?)(?:PACKAGE INCLUSIONS)",
        text,
        re.DOTALL | re.IGNORECASE,
    )
    if not section_m:
    ...
    for m in pattern.finditer(section):
        ...
        location = "Multi-city Australia" if len(hotelish) > 1 else "Australia"
        options.append(
            {
                "sort_order": num,
                "category": category,
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_au(text)
    tour_code = extract_tour_code(raw)
    destinations = extract_destinations_au(text)
    hotels = parse_au_hotel_options(text)
    if len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "Australia",
                "nights_label": nights,
                "room_type": "Deluxe Room",
                "meal_plan": "MAPAI (Breakfast & Dinner)",
                "stars": 5 if "ultra" in cat.lower() else 4,
    ...
    primary_loc = destinations.split("•")[0].strip() or "Australia"
    for hotel in hotels:
    ...
    included, excluded = parse_au_inclusions_exclusions(raw)
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_au(text)
    destinations = meta.get("destinations") or extract_destinations_au(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"State / Country: Australia | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_au(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_au(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_au_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_au_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_au(text)
    seo_desc = (
        f"Premium {duration} Australia package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for AU-005 Australia international domestic packages."""
...
'''
    ...
    builder_names = [f"build_au_{serial.split('-')[1]}" for serial in BUILDER_ORDER]
    footer = "\nAU_005_BUILDERS = [\n    " + ",\n    ".join(builder_names) + ",\n]\n"
