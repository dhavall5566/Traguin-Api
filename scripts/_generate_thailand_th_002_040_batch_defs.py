#!/usr/bin/env python3
"""Generate thailand_th_002_040_batch_defs.py from tmp_th_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_th_pdfs"
OUT_PATH = _SCRIPTS / "thailand_th_002_040_batch_defs.py"
PRICE_NOTE = "On Request (Premium Customised)"
BUILDER_ORDER = [f"TH-{i:03d}" for i in range(2, 41)]
...
_TH_FOOTER_LINE = re.compile(
    r"TRAGUIN (?:Premium Luxury Itinerary|Premium Holidays|Luxury Holidays|Luxury Proposals?|Luxury Proposal)"
    r"(?: • TH-\d{3})?(?: • www\.traguin\.in)?(?: Page \d+ of \d+)?(?:\s*—\s*[^\n]+)?\s*\d*",
    re.IGNORECASE,
)
_TH_FOOTER_FRAGMENT = re.compile(
    r"(?:TRAGUIN )?(?:Premium )?(?:Luxury )?(?:Holidays|Itinerary|Holiday Proposals?)"
    r"(?: • TH-\d{3})?(?: • www\.traguin\.in)?[^\n]*?(?=DAY\s*\d+)",
    re.IGNORECASE,
)
...
def clean_th_raw(raw: str) -> str:
    raw = _TH_FOOTER_LINE.sub("\n", raw)
    text = clean_raw(raw)
    ...
    text = _TH_FOOTER_LINE.sub("\n", text)
    text = _TH_FOOTER_FRAGMENT.sub("\n", text)
    text = _normalize_day_headers(text)
    text = re.sub(r"(DAY\s*\d+\s*\|\s*[^\n]+)\n\s*[—–-]\s*", r"\1 | ", text, flags=re.MULTILINE)
    return text


def extract_destinations_th(text: str) -> str:
    ...
    return re.sub(r"\s+", " ", m.group(1).strip()) if m else "Thailand"


def extract_tour_code(text: str) -> str:
    normalized = re.sub(r"TRAGUIN TOUR\s*\n\s*CODE:", "TRAGUIN TOUR CODE:", text, flags=re.I)
    block = re.search(
        r"TRAGUIN TOUR\s*CODE:\s*(.+?)(?=\s*(?:STATE|VEHICLE|MEAL PLAN|TRAGUIN PREMIUM|TOUR OVERVIEW|Create |Embark |Step |Welcome))",
        normalized,
        re.DOTALL | re.I,
    )
    if block:
    ...
    return m.group(1) if m else "TRG-THA-000"


def extract_title_th(text: str) -> str:
    header = re.search(
        r"TRAGUIN (?:PREMIUM|LUXURY|ULTIMATE)\s+(.+?)(?:\n\n|\n\d+\s+Nights|\nSERIAL|\nSTATE|\nCreate |\nEmbark |\nStep |\nWelcome to|\nTOUR OVERVIEW)",
        text,
    ...
    return "Thailand Premium Luxury Tour"


def extract_vehicle_meals_th(text: str) -> str:
    ...
    return "Private Luxury Executive Van / Buffet Breakfast"
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"th-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str) -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_th(text)} {extract_title_th(text)}".lower()
    candidates = [
        ("Family", ("family", "group")),
        ("Luxury", ("luxury", "premium", "bespoke")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Culture", ("culture", "heritage", "temple", "palace", "grand palace")),
        ("Beach", ("beach", "island", "coral", "phuket", "pattaya", "krabi", "marine")),
        ("Wildlife", ("wildlife", "safari", "zoo", "elephant")),
        ("Leisure", ("leisure", "senior", "relax")),
        ("Corporate", ("corporate", "mice", "conference", "delegate", "executive")),
        ("Adventure", ("adventure", "expedition", "thrill", "theme park")),
        ("Nature", ("nature", "reef", "rainforest", "mountain")),
    ]
    moods: list[str] = []
...
def parse_day_blocks_th(text: str) -> list[dict]:
    start = re.search(
        r"(?:THE DEFINITIVE DAY-WISE ITINERARY|YOUR BESPOKE DAY-WISE ITINERARY|DETAILED DAY-WISE ITINERARY|DAY-WISE ITINERARY|DAY WISE ITINERARY)",
        text,
        re.IGNORECASE,
    )
    if start:
...
def extract_category_th(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
        re.DOTALL | re.IGNORECASE,
...
def _default_th_inclusions() -> tuple[list[str], list[str]]:
    return (
        [
            "Handpicked premium accommodation as per selected hotel tier",
            "Private luxury vehicle transfers and sightseeing as per itinerary",
            "Daily buffet breakfast and curated meals as specified",
            "VIP airport fast-track reception and dedicated TRAGUIN concierge support",
        ],
        [
            "International airfare and Thailand visa fees",
            "Personal expenses, laundry, mini-bar, and tips",
...
def parse_th_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if not included and not excluded:
        return _default_th_inclusions()
    if not included:
        included = _default_th_inclusions()[0]
    if not excluded:
        excluded = _default_th_inclusions()[1]
    return included, excluded


def parse_th_hotel_options(text: str) -> list[dict]:
    """Parse Thailand PDF hotel option blocks (Option 01: Deluxe format)."""
    section_m = re.search(
        r"HANDPICKED (?:PREMIUM|ULTRA-LUXURY|LUXURY|ELITE)[^\n]*(?:\nACCOMMODATIONS|\nHOTELS)?[^\n]*\s*(.+?)(?:\"Creating Memories|PACKAGE INCLUSIONS|PREMIUM TRAVEL|\Z)",
        text,
        re.DOTALL | re.IGNORECASE,
    )
    if not section_m:
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    ...
    destinations = extract_destinations_th(text)
    hotels = parse_th_hotel_options(text)
    if len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "Thailand",
                "nights_label": nights,
                "room_type": "Deluxe Room",
    ...
    primary_loc = destinations.split("•")[0].strip() or "Thailand"
    for hotel in hotels:
        if hotel.get("location") in ("Jharkhand", "", "Haryana", "Multi-city Jharkhand"):
            hotel["location"] = primary_loc
        hotel["nights_label"] = _compact_nights_label(hotel.get("nights_label") or _jh._infer_nights(text))
    included, excluded = parse_th_inclusions_exclusions(raw)
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_th(text)
    destinations = meta.get("destinations") or extract_destinations_th(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"State / Country: Thailand | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_th(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_th(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_th_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_th_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_th(text)
    seo_desc = (
        f"Premium {duration} Thailand package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for TH-002 through TH-040 Thailand international domestic packages."""
...
'''
    ...
    builder_names = [f"build_th_{serial.split('-')[1]}" for serial in BUILDER_ORDER]
