#!/usr/bin/env python3
"""Insert JH-001 through JH-010 Jharkhand packages (no images, skip if exists)."""
...
from jharkhand_domestic_batch_defs import JHARKHAND_DOMESTIC_BUILDERS, JHARKHAND_SLUG
from models.destinations import Destination
from services.package_insert import insert_package_if_missing
from utils.db import commit_or_raise


def get_or_create_jharkhand_destination(db) -> Destination:
    destination = db.scalar(select(Destination).where(Destination.slug == JHARKHAND_SLUG))
...
def main() -> None:
    ...
    with SessionLocal() as db:
        destination = get_or_create_jharkhand_destination(db)
        dest_id = str(destination.id)

        for builder in JHARKHAND_DOMESTIC_BUILDERS:
