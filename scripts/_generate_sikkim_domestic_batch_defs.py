#!/usr/bin/env python3
"""Generate sikkim_domestic_batch_defs.py from tmp_sk_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_sk_pdfs"
OUT_PATH = _SCRIPTS / "sikkim_domestic_batch_defs.py"
PRICE_NOTE = "On Request (Premium Customised)"
BUILDER_ORDER = [f"SK-{i:03d}" for i in range(1, 16)]
...
def extract_destinations_sikkim(text: str) -> str:
    ...
    return re.sub(r"\s+", " ", m.group(1).strip()) if m else "Gangtok • Sikkim"
...
def extract_tour_code(text: str) -> str:
    ...
    return m.group(1) if m else "TRG-SIK-000"


def extract_title_sikkim(text: str) -> str:
    patterns = [
        r"TRAGUIN PREMIUM SIKKIM TOUR PACKAGE\s*\n(.+?)(?:\nWelcome|\nTOUR OVERVIEW|TRAGUIN Premium)",
        r"TRAGUIN PREMIUM SIKKIM TOUR\s*\nPACKAGE\s*\n(.+?)(?:\nWelcome|\nTOUR OVERVIEW|TRAGUIN Premium)",
        r"TRAGUIN PREMIUM SIKKIM TOUR PACKAGE\s*\n(.+)",
    ]
    for pat in patterns:
        ...
        if m:
            ...
            if len(title) > 12:
                return title
    return "Sikkim Premium Luxury Tour"


def extract_vehicle_meals_sikkim(text: str) -> str:
    flat = re.sub(r"\s+", " ", text)
    for label in (
        "VEHICLE / MEALS:",
        "VEHICLE & MEALS:",
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"sk-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str) -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category(text)} {extract_title_sikkim(text)}".lower()
    candidates = [
        ("Family", ("family", "group")),
        ("Luxury", ("luxury", "premium", "bespoke")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Adventure", ("adventure", "expedition", "thrill")),
        ("Nature", ("nature", "alpine", "mountain", "lake", "valley")),
        ("Culture", ("culture", "heritage", "monastery", "spiritual")),
        ("Leisure", ("leisure", "senior")),
        ("Educational", ("educational", "school", "students")),
    ]
    moods: list[str] = []
...
def parse_day_blocks_sikkim(text: str) -> list[dict]:
    start = re.search(
...
def parse_sikkim_hotel_options(text: str) -> list[dict]:
    """Parse Sikkim PDF table-style hotel option blocks (4 tiers)."""
    section_m = re.search(
        r"(?:HANDPICKED (?:PREMIUM )?(?:HOTEL|ACCOMMODATION)(?: OPTIONS)?|"
    ...
    for m in pattern.finditer(section):
        ...
        block = re.sub(r"^(Category|Gangtok|Lachung|Lachen|Room Category).*$", "", block, flags=re.I)
        block = block.strip(" /")
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_sikkim(text)
    tour_code = extract_tour_code(raw)
    destinations = extract_destinations_sikkim(text)
    hotels = parse_hotel_options(text)
    if len(hotels) < 4:
        alt = parse_sikkim_hotel_options(text)
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "Sikkim",
                "nights_label": nights,
    ...
    primary_loc = destinations.split("•")[0].strip() or "Sikkim"
    for hotel in hotels:
    ...
    included, excluded = parse_inclusions_exclusions(raw)
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    destinations = meta.get("destinations") or extract_destinations_sikkim(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"State / Country: Sikkim / India | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_sikkim(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_sikkim(text))
    hotels = meta["hotels"]
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_sikkim(text)
    seo_desc = (
        f"Premium {duration} Sikkim package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for SK-001 through SK-015 Sikkim domestic packages."""
...
'''
    ...
    builder_names = [f"build_sk_{serial.split('-')[1]}" for serial in BUILDER_ORDER]
