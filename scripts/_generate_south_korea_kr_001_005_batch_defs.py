#!/usr/bin/env python3
"""Generate south_korea_kr_001_005_batch_defs.py from tmp_kr_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_kr_pdfs"
OUT_PATH = _SCRIPTS / "south_korea_kr_001_005_batch_defs.py"
PRICE_NOTE = "On Request (Premium South Korea Experience)"

TITLE_MAP = {
    "KR-001": "Seoul Highlights Family Tour",
    "KR-002": "Romantic Korea Honeymoon",
    "KR-003": "South Korea Luxury Tour",
    "KR-005": "Premium South Korea Tour",
}

SLUG_MAP = {
    "KR-001": "kr-001-seoul-highlights-family-tour",
    "KR-002": "kr-002-romantic-korea-honeymoon",
    "KR-003": "kr-003-south-korea-luxury-tour",
    "KR-005": "kr-005-premium-south-korea-tour",
}

ITIN_SLUG_MAP = {
    "KR-001": "kr-001-seoul-highlights-itinerary",
    "KR-002": "kr-002-romantic-korea-itinerary",
    "KR-003": "kr-003-south-korea-luxury-itinerary",
    "KR-005": "kr-005-premium-south-korea-itinerary",
}

BUILDER_ORDER = ["KR-001", "KR-002", "KR-003", "KR-005"]
...
_KR_FOOTER_LINE = re.compile(
    ...
    r"(?: \| KR-\d{3})?(?: • KR-\d{3})?(?: • www\.traguin\.in)?(?: Page \d+ of \d+|\s+\d+)?(?:\s*—\s*[^\n]+)?\s*\d*",
    re.IGNORECASE,
)
_KR_FOOTER_FRAGMENT = re.compile(
    r"(?:TRAGUIN )?(?:Premium )?(?:Luxury )?(?:Itinerary|Travel|Holidays|Holiday Proposals?|Family Vacations|Honeymoon Experts|World)"
    r"(?: \| KR-\d{3})?(?: • KR-\d{3})?(?: • www\.traguin\.in)?[^\n]*?(?=DAY\s*\d+)",
    re.IGNORECASE,
)
...
def normalize_kr_raw(raw: str) -> str:
    ...
    raw = re.sub(
        r"TRAGUIN Premium (?:Itinerary|Travel)\s*\|\s*KR-\d{3}\s*(?:Page\s*)?\d*",
        "\n",
        raw,
        flags=re.I,
    )
...
def clean_kr_raw(raw: str) -> str:
    raw = normalize_kr_raw(raw)
    raw = _KR_FOOTER_LINE.sub("\n", raw)
    text = clean_raw(raw)
    ...
    text = _KR_FOOTER_LINE.sub("\n", text)
    text = _KR_FOOTER_FRAGMENT.sub("\n", text)
    text = _normalize_day_headers(text)
    text = re.sub(r"(DAY\s*\d+\s*\|\s*[^\n]+)\n\s*[—–-]\s*", r"\1 | ", text, flags=re.MULTILINE)
    return text


def extract_destinations_kr(text: str) -> str:
...
def extract_tour_code(text: str) -> str:
    ...
    for pat in (
        ...
        if m:
            ...
            return code
    m = re.search(r"Serial Code:\s*KR-\d{3}", flat, re.I)
    if m:
        return f"KR-{m.group(0).split('-')[-1]}-PREM"
    return "TRG-KOR-000"


def extract_title_kr(text: str, serial: str = "") -> str:
    ...
    if tagline:
        return re.sub(r"\s+", " ", tagline.group(1).strip())
    return "South Korea Premium Luxury Tour"


def extract_vehicle_meals_kr(text: str) -> str:
    ...
    return "Private Luxury Executive Sedan / Buffet Breakfast & Curated Dinners"
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"kr-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str, serial: str = "") -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_kr(text)} {extract_title_kr(text, serial)}".lower()
    candidates = [
        ("Family", ("family", "group", "multi-generational")),
        ("Luxury", ("luxury", "premium", "bespoke", "ultra")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Culture", ("culture", "heritage", "palace", "hanbok", "gyeongbokgung", "temple")),
        ("Nature", ("jeju", "nami", "island", "coastal", "scenic", "waterfall")),
        ("Adventure", ("adventure", "lotte world", "ktx", "theme park", "universal")),
        ("Food & Dining", ("michelin", "korean bbq", "culinary", "gourmet", "dining")),
        ("Leisure", ("leisure", "senior", "relax", "family", "spa")),
    ]
    moods: list[str] = []
...
def parse_day_blocks_kr(text: str) -> list[dict]:
    start = re.search(
...
def extract_category_kr(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
        re.DOTALL | re.IGNORECASE,
...
def _default_kr_inclusions() -> tuple[list[str], list[str]]:
    return (
        [
            "Premium handpicked accommodation across South Korea",
            "Private luxury airport transfers and dedicated chauffeur-driven executive transport",
            "KTX high-speed rail and domestic connections as per itinerary",
            "TRAGUIN 24/7 dedicated bilingual on-ground concierge support",
        ],
        [
            "International airfare and South Korea visa or K-ETA fees",
...
def parse_short_inclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if included:
        _, excluded = _default_kr_inclusions()
        return included, excluded
    return _default_kr_inclusions()


def parse_kr_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if not included and not excluded:
        return _default_kr_inclusions()
    if not included:
        included = _default_kr_inclusions()[0]
    if not excluded:
        excluded = _default_kr_inclusions()[1]
    return included, excluded


def parse_kr_hotel_options(text: str) -> list[dict]:
    """Parse South Korea PDF hotel option blocks."""
    section_m = re.search(
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_kr(text, serial)
    tour_code = extract_tour_code(normalize_kr_raw(raw))
    destinations = extract_destinations_kr(text)
    hotels = parse_kr_hotel_options(text)
    if len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "South Korea",
                "nights_label": nights,
                "room_type": "Deluxe Room",
    ...
    primary_loc = destinations.split("•")[0].strip() or "South Korea"
    for hotel in hotels:
    ...
    if serial in {"KR-001", "KR-005"}:
        included, excluded = parse_short_inclusions(text)
        if len(included) < 3:
            included, excluded = parse_kr_inclusions_exclusions(raw)
    else:
        included, excluded = parse_kr_inclusions_exclusions(raw)
    return {
        "serial": serial,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_kr(text)
    destinations = meta.get("destinations") or extract_destinations_kr(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"Country: South Korea, Asia | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_kr(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_kr(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_kr_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_kr_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_kr(text)
    seo_desc = (
        f"Premium {duration} South Korea package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for KR-001, KR-002, KR-003, and KR-005 South Korea international packages."""
...
'''
    ...
    builder_names = [f"build_kr_{serial.split('-')[1]}" for serial in BUILDER_ORDER if (PDF_DIR / f"{serial}.txt").exists()]
