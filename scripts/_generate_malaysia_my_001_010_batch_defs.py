#!/usr/bin/env python3
"""Generate malaysia_my_001_010_batch_defs.py from tmp_my_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_my_pdfs"
OUT_PATH = _SCRIPTS / "malaysia_my_001_010_batch_defs.py"
...
TITLE_MAP = {
    ...
    "MY-004": "Langkawi Romance Package",
    "MY-005": "Malaysia Luxury Tour",
    "MY-006": "Malaysia Shopping Tour",
    "MY-007": "Malaysia Relax Senior Tour",
    "MY-008": "Malaysia Family Fun Tour",
    "MY-009": "Langkawi Adventure Tour",
    "MY-010": "Grand Malaysia Luxury Tour",
}

SLUG_MAP = {
    "MY-001": "my-001-kuala-lumpur-genting-family-tour",
    "MY-002": "my-002-malaysia-discovery-family-tour",
    "MY-003": "my-003-kuala-lumpur-langkawi-family-tour",
    "MY-004": "my-004-langkawi-romance-package",
    "MY-005": "my-005-malaysia-luxury-tour",
    "MY-006": "my-006-malaysia-shopping-tour",
    "MY-007": "my-007-malaysia-relax-senior-tour",
    "MY-008": "my-008-malaysia-family-fun-tour",
    "MY-009": "my-009-langkawi-adventure-tour",
    "MY-010": "my-010-grand-malaysia-luxury-tour",
}

ITIN_SLUG_MAP = {
    "MY-001": "my-001-kuala-lumpur-genting-itinerary",
    "MY-002": "my-002-malaysia-discovery-itinerary",
    "MY-003": "my-003-kuala-lumpur-langkawi-itinerary",
    "MY-004": "my-004-langkawi-romance-itinerary",
    "MY-005": "my-005-malaysia-luxury-itinerary",
    "MY-006": "my-006-malaysia-shopping-itinerary",
    "MY-007": "my-007-malaysia-relax-senior-itinerary",
    "MY-008": "my-008-malaysia-family-fun-itinerary",
    "MY-009": "my-009-langkawi-adventure-itinerary",
    "MY-010": "my-010-grand-malaysia-luxury-itinerary",
}

BUILDER_ORDER = [f"MY-{i:03d}" for i in range(1, 11)]
...
_MY_FOOTER_LINE = re.compile(
    ...
    r"(?: \| MY-\d{3})?(?: • MY-\d{3})?(?: • www\.traguin\.in)?(?: Page \d+ of \d+|\s+\d+)?(?:\s*—\s*[^\n]+)?\s*\d*",
    re.IGNORECASE,
)
_MY_FOOTER_FRAGMENT = re.compile(
    r"(?:TRAGUIN )?(?:Premium )?(?:Luxury )?(?:Itinerary|Travel|Holidays|Holiday Proposals?|Family Vacations|Honeymoon Experts|World)"
    r"(?: \| MY-\d{3})?(?: • MY-\d{3})?(?: • www\.traguin\.in)?[^\n]*?(?=DAY\s*\d+)",
    re.IGNORECASE,
)
...
def normalize_my_raw(raw: str) -> str:
    ...
    raw = re.sub(
        r"TRAGUIN Premium (?:Itinerary|Travel)\s*\|\s*MY-\d{3}\s*(?:Page\s*)?\d*",
        "\n",
        raw,
        flags=re.I,
    )
...
def clean_my_raw(raw: str) -> str:
    raw = normalize_my_raw(raw)
    raw = _MY_FOOTER_LINE.sub("\n", raw)
    text = clean_raw(raw)
    ...
    text = _MY_FOOTER_LINE.sub("\n", text)
    text = _MY_FOOTER_FRAGMENT.sub("\n", text)
    text = _normalize_day_headers(text)
    text = re.sub(r"(DAY\s*\d+\s*\|\s*[^\n]+)\n\s*[—–-]\s*", r"\1 | ", text, flags=re.MULTILINE)
    return text


def extract_destinations_my(text: str) -> str:
    ...
    return "Malaysia, Asia"


def extract_tour_code(text: str) -> str:
    flat = re.sub(r"\s+", " ", normalize_my_raw(text))
    for pat in (
        ...
        if m:
            return re.sub(r"\s+", "-", m.group(1).strip())
    m = re.search(r"Serial Code:\s*MY-\d{3}", flat, re.I)
    if m:
        return f"TRG-MAL-{m.group(0).split('-')[-1]}-2026"
    return "TRG-MAL-000"


def extract_title_my(text: str, serial: str = "") -> str:
    ...
    if tagline:
        return re.sub(r"\s+", " ", tagline.group(1).strip())
    return "Malaysia Premium Luxury Tour"


def extract_vehicle_meals_my(text: str) -> str:
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"my-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str, serial: str = "") -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_my(text)} {extract_title_my(text, serial)}".lower()
    candidates = [
        ("Family", ("family", "group", "multi-generational")),
        ("Luxury", ("luxury", "premium", "bespoke", "ultra")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Culture", ("culture", "heritage", "batu caves", "penang", "malacca", "george town")),
        ("Nature", ("langkawi", "cameron", "mangrove", "island", "highlands", "scenic")),
        ("Adventure", ("adventure", "theme park", "genting", "sunway", "skybridge", "jet ski")),
        ("Beach", ("beach", "langkawi", "yacht", "coastal", "island")),
        ("Leisure", ("leisure", "senior", "relax", "family", "shopping", "spa")),
    ]
    moods: list[str] = []
...
def parse_day_blocks_my(text: str) -> list[dict]:
    start = re.search(
...
def extract_category_my(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
        re.DOTALL | re.IGNORECASE,
...
def _default_my_inclusions() -> tuple[list[str], list[str]]:
    return (
        [
            "Premium handpicked accommodation across Malaysia",
            "Private luxury airport transfers and dedicated chauffeur-driven van",
            "Domestic flights and inter-city transfers as per curated itinerary",
            "TRAGUIN 24/7 dedicated on-ground concierge support",
        ],
        [
            "International airfare and Malaysia visa fees",
...
def parse_short_inclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if included:
        _, excluded = _default_my_inclusions()
        return included, excluded
    return _default_my_inclusions()


def parse_my_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if not included and not excluded:
        return _default_my_inclusions()
    if not included:
        included = _default_my_inclusions()[0]
    if not excluded:
        excluded = _default_my_inclusions()[1]
    return included, excluded


def parse_my_hotel_options(text: str) -> list[dict]:
    """Parse Malaysia PDF hotel option blocks."""
    section_m = re.search(
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_my(text, serial)
    tour_code = extract_tour_code(normalize_my_raw(raw))
    destinations = extract_destinations_my(text)
    hotels = parse_my_hotel_options(text)
    if len(hotels) < 4:
        alt = parse_hotel_options(text)
        if len(alt) > len(hotels):
            hotels = alt
    nights = _compact_nights_label(_jh._infer_nights(text))
    primary_loc = destinations.split("•")[0].strip() or "Malaysia"
    if 0 < len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "category": cat,
                "name": f"TRAGUIN Handpicked {cat} Property",
                "location": destinations.split("•")[0].strip() or "Malaysia",
                "nights_label": nights,
                "room_type": "Deluxe Room",
                "meal_plan": "MAPAI (Breakfast & Dinner)",
                "stars": 5 if "ultra" in cat.lower() else 4,
    ...
    included, excluded = parse_my_inclusions_exclusions(raw)
    if len(included) < 3:
        ...
        if len(short_inc) >= 3:
            ...
            included, excluded = _default_my_inclusions()
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_my(text)
    destinations = meta.get("destinations") or extract_destinations_my(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"Country: Malaysia, Asia | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_my(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_my(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_my_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_my_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_my(text)
    seo_desc = (
        f"Premium {duration} Malaysia package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for MY-001 through MY-010 Malaysia international packages."""
...
'''
    ...
    builder_names = [f"build_my_{serial.split('-')[1]}" for serial in BUILDER_ORDER if (PDF_DIR / f"{serial}.txt").exists()]
