#!/usr/bin/env python3
"""Generate uttar_pradesh_up_002_010_batch_defs.py from tmp_up_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_up_pdfs"
OUT_PATH = _SCRIPTS / "uttar_pradesh_up_002_010_batch_defs.py"
PRICE_NOTE = "On Request (Premium Customised)"
BUILDER_ORDER = [f"UP-{i:03d}" for i in range(2, 11)]
...
def clean_up_raw(raw: str) -> str:
    text = clean_raw(raw)
    text = re.sub(
        r"TRAGUIN (?:Premium Luxury Itinerary|Luxury Proposal) — UP-\d{3}\s*\d+",
        " ",
        text,
        flags=re.IGNORECASE,
    )
...
def extract_destinations_up(text: str) -> str:
    ...
    return re.sub(r"\s+", " ", m.group(1).strip()) if m else "Uttar Pradesh"
...
def extract_tour_code(text: str) -> str:
    ...
    return m.group(1) if m else "TRG-UP-000"


def extract_title_up(text: str) -> str:
    patterns = [
        r"TRAGUIN PREMIUM UTTAR PRADESH TOUR\s*\nPACKAGE\s*\n(.+?)(?:\nTRAGUIN Premium|\nTRAGUIN Luxury|\nWelcome|\nTOUR OVERVIEW)",
        r"TRAGUIN PREMIUM UTTAR PRADESH TOUR PACKAGE\s*\n(.+?)(?:\nTRAGUIN Premium|\nTRAGUIN Luxury|\nWelcome|\nTOUR OVERVIEW)",
        r"TRAGUIN PREMIUM UTTAR PRADESH TOUR\s*\nPACKAGE\s*\n(.+)",
    ]
    for pat in patterns:
        ...
        if m:
            ...
            if len(title) > 12:
                return title
    return "Uttar Pradesh Premium Luxury Tour"


def extract_vehicle_meals_up(text: str) -> str:
    flat = re.sub(r"\s+", " ", text)
    for label in (
        "VEHICLE / MEALS:",
        "VEHICLE & MEALS:",
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"up-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str) -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_up(text)} {extract_title_up(text)}".lower()
    candidates = [
        ("Family", ("family", "group")),
        ...
        ("Spiritual", ("spiritual", "pilgrimage", "dham", "temple", "ghat", "sangam", "divine")),
...
def parse_day_blocks_up(text: str) -> list[dict]:
    start = re.search(
...
def extract_category_up(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
...
def parse_up_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    cleaned = _jh.PAGE_RE.sub(" ", text)
    cleaned = re.sub(
        r"TRAGUIN (?:Premium Luxury Itinerary|Corporate Luxury Proposal) — UP-\d{3}\s*\d+",
        " ",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def parse_up_hotel_options(text: str) -> list[dict]:
    """Parse Uttar Pradesh PDF hotel option blocks (4 tiers, single or multi-city)."""
    section_m = re.search(
    ...
    for m in pattern.finditer(section):
        ...
        for ln in m.group(3).splitlines():
            ...
            if ln in ("Room Category Offered", "Offered", "Meal Plan Included"):
                continue
            if re.match(
                r"^(Agra|Mathura|Varanasi|Lucknow|Prayagraj|Ayodhya).*(?:Stay|Options|Nights)",
                ln,
                re.I,
            ):
                continue
        ...
        location = "Multi-city Uttar Pradesh" if len(hotelish) > 1 else "Uttar Pradesh"
        options.append(
            {
                "sort_order": num,
                "category": category,
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_up(text)
    tour_code = extract_tour_code(raw)
    destinations = extract_destinations_up(text)
    hotels = parse_up_hotel_options(text)
    if len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "Uttar Pradesh",
                "nights_label": nights,
                "room_type": "Deluxe Room",
                "meal_plan": "MAPAI (Breakfast & Dinner)",
    ...
    primary_loc = destinations.split("•")[0].strip() or "Uttar Pradesh"
    for hotel in hotels:
    ...
    included, excluded = parse_up_inclusions_exclusions(raw)
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_up(text)
    destinations = meta.get("destinations") or extract_destinations_up(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"State / Country: Uttar Pradesh / India | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_up(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_up(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_up_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_up_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_up(text)
    seo_desc = (
        f"Premium {duration} Uttar Pradesh package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for UP-002 through UP-010 Uttar Pradesh domestic packages."""
...
'''
    ...
    builder_names = [f"build_up_{serial.split('-')[1]}" for serial in BUILDER_ORDER]
