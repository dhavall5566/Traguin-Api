#!/usr/bin/env python3
"""Generate japan_jp_001_005_batch_defs.py from tmp_jp_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_jp_pdfs"
OUT_PATH = _SCRIPTS / "japan_jp_001_005_batch_defs.py"
PRICE_NOTE = "On Request (Premium Japan Experience)"

TITLE_MAP = {
    "JP-001": "Japan Highlights Family Tour",
    "JP-002": "Japan Family Expedition",
    "JP-003": "Romantic Japan Honeymoon",
    "JP-004": "Luxury Japan Showcase",
    "JP-005": "Grand Japan Tour",
}

SLUG_MAP = {
    "JP-001": "jp-001-japan-highlights-family-tour",
    "JP-002": "jp-002-japan-family-expedition",
    "JP-003": "jp-003-romantic-japan-honeymoon",
    "JP-004": "jp-004-luxury-japan-showcase",
    "JP-005": "jp-005-grand-japan-tour",
}

ITIN_SLUG_MAP = {
    "JP-001": "jp-001-japan-highlights-itinerary",
    "JP-002": "jp-002-japan-family-expedition-itinerary",
    "JP-003": "jp-003-romantic-japan-itinerary",
    "JP-004": "jp-004-luxury-japan-itinerary",
    "JP-005": "jp-005-grand-japan-itinerary",
}

BUILDER_ORDER = [f"JP-{i:03d}" for i in range(1, 6)]
...
_JP_FOOTER_LINE = re.compile(
    r"TRAGUIN (?:Premium Luxury Itinerary|Premium Holidays|Luxury Holidays|Luxury Proposals?|Luxury Proposal|Family Vacations|Honeymoon Experts|World)"
    r"(?: • JP-\d{3})?(?: • www\.traguin\.in)?(?: Page \d+ of \d+|\s+\d+)?(?:\s*—\s*[^\n]+)?\s*\d*",
    re.IGNORECASE,
)
_JP_FOOTER_FRAGMENT = re.compile(
    r"(?:TRAGUIN )?(?:Premium )?(?:Luxury )?(?:Holidays|Itinerary|Holiday Proposals?|Family Vacations|Honeymoon Experts|World)"
    r"(?: • JP-\d{3})?(?: • www\.traguin\.in)?[^\n]*?(?=DAY\s*\d+)",
    re.IGNORECASE,
)
...
def normalize_jp_raw(raw: str) -> str:
...
def clean_jp_raw(raw: str) -> str:
    raw = normalize_jp_raw(raw)
    raw = _JP_FOOTER_LINE.sub("\n", raw)
    text = clean_raw(raw)
    ...
    text = _JP_FOOTER_LINE.sub("\n", text)
    text = _JP_FOOTER_FRAGMENT.sub("\n", text)
    text = _normalize_day_headers(text)
    text = re.sub(r"(DAY\s*\d+\s*\|\s*[^\n]+)\n\s*[—–-]\s*", r"\1 | ", text, flags=re.MULTILINE)
    return text


def extract_destinations_jp(text: str) -> str:
...
def extract_tour_code(text: str) -> str:
    flat = re.sub(r"\s+", " ", normalize_jp_raw(text))
    for pat in (
    ...
    m = re.search(r"Serial Code:\s*JP-\d{3}", flat, re.I)
    if m:
        return f"TRAGUIN-JPN-{m.group(0).split('-')[-1]}"
    return "TRAGUIN-JPN-000"


def extract_title_jp(text: str, serial: str = "") -> str:
    ...
    if tagline:
        return re.sub(r"\s+", " ", tagline.group(1).strip())
    return "Japan Premium Luxury Tour"


def extract_vehicle_meals_jp(text: str) -> str:
    ...
    return "Private Luxury Executive Van / Buffet Breakfast & Curated Dinners"
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"jp-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str, serial: str = "") -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_jp(text)} {extract_title_jp(text, serial)}".lower()
    candidates = [
        ("Family", ("family", "group", "multi-generational")),
        ("Luxury", ("luxury", "premium", "bespoke", "ultra")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Culture", ("culture", "heritage", "shrine", "temple", "kyoto", "geisha", "torii")),
        ("Nature", ("mt. fuji", "fuji", "hakone", "onsen", "bamboo", "scenic")),
        ("Adventure", ("adventure", "universal", "helicopter", "bullet train", "shinkansen")),
        ("Food & Dining", ("sushi", "michelin", "culinary", "izakaya", "ramen")),
        ("Leisure", ("leisure", "senior", "relax", "family")),
    ]
    moods: list[str] = []
...
def parse_day_blocks_jp(text: str) -> list[dict]:
    start = re.search(
...
def extract_category_jp(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
        re.DOTALL | re.IGNORECASE,
...
def _default_jp_inclusions() -> tuple[list[str], list[str]]:
    return (
        [
            "Premium handpicked accommodation across Japan",
            "Private luxury airport transfers and dedicated chauffeur-driven executive van",
            "First-class Shinkansen bullet train tickets with luggage forwarding",
            "TRAGUIN 24/7 dedicated bilingual on-ground concierge support",
        ],
        [
            "International airfare and Japan visa fees",
...
def parse_short_inclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if included:
        _, excluded = _default_jp_inclusions()
        return included, excluded
    return _default_jp_inclusions()


def parse_jp_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if not included and not excluded:
        return _default_jp_inclusions()
    if not included:
        included = _default_jp_inclusions()[0]
    if not excluded:
        excluded = _default_jp_inclusions()[1]
    return included, excluded


def parse_jp_hotel_options(text: str) -> list[dict]:
    """Parse Japan PDF hotel option blocks."""
    section_m = re.search(
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_jp(text, serial)
    tour_code = extract_tour_code(normalize_jp_raw(raw))
    destinations = extract_destinations_jp(text)
    hotels = parse_jp_hotel_options(text)
    if len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "location": destinations.split("•")[0].strip() or "Japan",
                "nights_label": nights,
                "room_type": "Deluxe Room",
    ...
    primary_loc = destinations.split("•")[0].strip() or "Japan"
    for hotel in hotels:
    ...
    included, excluded = parse_jp_inclusions_exclusions(raw)
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_jp(text)
    destinations = meta.get("destinations") or extract_destinations_jp(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"Country: Japan, East Asia | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_jp(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_jp(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_jp_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_jp_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_jp(text)
    seo_desc = (
        f"Premium {duration} Japan package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for JP-001 through JP-005 Japan international packages."""
...
'''
    ...
    builder_names = [f"build_jp_{serial.split('-')[1]}" for serial in BUILDER_ORDER if (PDF_DIR / f"{serial}.txt").exists()]
