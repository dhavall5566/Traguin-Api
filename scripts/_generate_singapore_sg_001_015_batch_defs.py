#!/usr/bin/env python3
"""Generate singapore_sg_001_015_batch_defs.py from tmp_sg_pdfs text sources."""
...
PDF_DIR = _SCRIPTS.parent / "tmp_sg_pdfs"
OUT_PATH = _SCRIPTS / "singapore_sg_001_015_batch_defs.py"
PRICE_NOTE = "On Request (Premium Singapore Experience)"

TITLE_MAP = {
    "SG-001": "Singapore Highlights Family Tour",
    "SG-002": "Singapore & Sentosa Family Tour",
    "SG-003": "Singapore Universal Studios Special",
    "SG-004": "Singapore & Melbourne Family Tour",
    "SG-005": "Luxury Singapore Tour",
    "SG-006": "Singapore Explorer",
    "SG-007": "Singapore & Malaysia Tour",
    "SG-008": "Singapore Ladies Shopping Tour",
    "SG-009": "Singapore Leisure Tour",
    "SG-010": "Singapore Educational Tour",
    "SG-011": "Singapore Sentosa Fun",
    "SG-012": "Marina Bay Luxury Tour",
    "SG-013": "Singapore Deluxe Family Tour",
    "SG-014": "Singapore Corporate Tour",
    "SG-015": "Grand Singapore Family Tour",
}

SLUG_MAP = {
    "SG-001": "sg-001-singapore-highlights-family-tour",
    "SG-002": "sg-002-singapore-sentosa-family-tour",
    "SG-003": "sg-003-singapore-universal-studios-special",
    "SG-004": "sg-004-singapore-melbourne-family-tour",
    "SG-005": "sg-005-luxury-singapore-tour",
    "SG-006": "sg-006-singapore-explorer-tour",
    "SG-007": "sg-007-singapore-malaysia-tour",
    "SG-008": "sg-008-singapore-ladies-shopping-tour",
    "SG-009": "sg-009-singapore-leisure-tour",
    "SG-010": "sg-010-singapore-educational-tour",
    "SG-011": "sg-011-singapore-sentosa-fun-tour",
    "SG-012": "sg-012-marina-bay-luxury-tour",
    "SG-013": "sg-013-singapore-deluxe-family-tour",
    "SG-014": "sg-014-singapore-corporate-tour",
    "SG-015": "sg-015-grand-singapore-family-tour",
}

ITIN_SLUG_MAP = {
    "SG-001": "sg-001-singapore-highlights-itinerary",
    "SG-002": "sg-002-singapore-sentosa-itinerary",
    "SG-003": "sg-003-singapore-universal-studios-itinerary",
    "SG-004": "sg-004-singapore-melbourne-itinerary",
    "SG-005": "sg-005-luxury-singapore-itinerary",
    "SG-006": "sg-006-singapore-explorer-itinerary",
    "SG-007": "sg-007-singapore-malaysia-itinerary",
    "SG-008": "sg-008-singapore-ladies-shopping-itinerary",
    "SG-009": "sg-009-singapore-leisure-itinerary",
    "SG-010": "sg-010-singapore-educational-itinerary",
    "SG-011": "sg-011-singapore-sentosa-fun-itinerary",
    "SG-012": "sg-012-marina-bay-luxury-itinerary",
    "SG-013": "sg-013-singapore-deluxe-family-itinerary",
    "SG-014": "sg-014-singapore-corporate-itinerary",
    "SG-015": "sg-015-grand-singapore-family-itinerary",
}

BUILDER_ORDER = [f"SG-{i:03d}" for i in range(1, 16)]
...
_SG_FOOTER_LINE = re.compile(
    ...
    r"(?: \| SG-\d{3})?(?: • SG-\d{3})?(?: • www\.traguin\.in)?(?: Page \d+ of \d+|\s+\d+)?(?:\s*—\s*[^\n]+)?\s*\d*",
    re.IGNORECASE,
)
_SG_FOOTER_FRAGMENT = re.compile(
    r"(?:TRAGUIN )?(?:Premium )?(?:Luxury )?(?:Itinerary|Travel|Holidays|Holiday Proposals?|Family Vacations|Honeymoon Experts|World)"
    r"(?: \| SG-\d{3})?(?: • SG-\d{3})?(?: • www\.traguin\.in)?[^\n]*?(?=DAY\s*\d+)",
    re.IGNORECASE,
)
...
def normalize_sg_raw(raw: str) -> str:
    ...
    raw = re.sub(
        r"TRAGUIN Premium (?:Itinerary|Travel)\s*\|\s*SG-\d{3}\s*(?:Page\s*)?\d*",
        "\n",
        raw,
        flags=re.I,
    )
...
def clean_sg_raw(raw: str) -> str:
    raw = normalize_sg_raw(raw)
    raw = _SG_FOOTER_LINE.sub("\n", raw)
    text = clean_raw(raw)
    ...
    text = _SG_FOOTER_LINE.sub("\n", text)
    text = _SG_FOOTER_FRAGMENT.sub("\n", text)
    text = _normalize_day_headers(text)
    text = re.sub(r"(DAY\s*\d+\s*\|\s*[^\n]+)\n\s*[—–-]\s*", r"\1 | ", text, flags=re.MULTILINE)
    return text


def extract_destinations_sg(text: str) -> str:
    ...
    return "Singapore, Asia"


def extract_tour_code(text: str) -> str:
    flat = re.sub(r"\s+", " ", normalize_sg_raw(text))
    for pat in (
        ...
        if m:
            return re.sub(r"\s+", "-", m.group(1).strip())
    m = re.search(r"Serial Code:\s*SG-\d{3}", flat, re.I)
    if m:
        return f"TRG-SIN-{m.group(0).split('-')[-1]}-2026"
    return "TRG-SIN-000"


def extract_title_sg(text: str, serial: str = "") -> str:
    ...
    if tagline:
        return re.sub(r"\s+", " ", tagline.group(1).strip())
    return "Singapore Premium Luxury Tour"


def extract_vehicle_meals_sg(text: str) -> str:
...
def make_slug(serial: str, title: str) -> str:
    ...
    return f"sg-{num}-{base}"[:118].rstrip("-")


def infer_moods(text: str, serial: str = "") -> list[str]:
    blob = f"{extract_ideal_for(text)} {extract_category_sg(text)} {extract_title_sg(text, serial)}".lower()
    candidates = [
        ("Family", ("family", "group", "multi-generational")),
        ("Luxury", ("luxury", "premium", "bespoke", "ultra")),
        ("Honeymoon", ("honeymoon", "romantic", "couple", "love")),
        ("Culture", ("culture", "heritage", "marina bay", "chinatown", "little india")),
        ("Nature", ("gardens by the bay", "night safari", "wildlife", "nature", "scenic")),
        ("Adventure", ("adventure", "universal studios", "sentosa", "theme park", "cove")),
        ("Luxury", ("luxury", "yacht", "michelin", "vip", "ultra")),
...
def parse_day_blocks_sg(text: str) -> list[dict]:
    start = re.search(
...
def extract_category_sg(text: str) -> str:
    m = re.search(
        r"CATEGORY:\s*(.+?)(?:\nDURATION|\nDESTINATIONS|\nIDEAL|\nBEST|\nSTARTING|\nTRAGUIN)",
        text,
        re.DOTALL | re.IGNORECASE,
...
def _default_sg_inclusions() -> tuple[list[str], list[str]]:
    return (
        [
            "Premium handpicked accommodation across Singapore",
            "Private luxury airport transfers and dedicated chauffeur-driven van",
            "International and connecting flights as per curated itinerary (where applicable)",
            "TRAGUIN 24/7 dedicated on-ground concierge support",
        ],
        [
            "International airfare and Singapore visa fees",
...
def parse_short_inclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if included:
        _, excluded = _default_sg_inclusions()
        return included, excluded
    return _default_sg_inclusions()


def parse_sg_inclusions_exclusions(text: str) -> tuple[list[str], list[str]]:
    ...
    if not included and not excluded:
        return _default_sg_inclusions()
    if not included:
        included = _default_sg_inclusions()[0]
    if not excluded:
        excluded = _default_sg_inclusions()[1]
    return included, excluded


def parse_sg_hotel_options(text: str) -> list[dict]:
    """Parse Singapore PDF hotel option blocks."""
    section_m = re.search(
    ...
    if not section_m:
        section_m = re.search(
            r"HANDPICKED PREMIUM (?:FAMILY )?(?:ACCOMMODATIONS|STAYS)\s*(.+?)(?:\"Creating Memories|PACKAGE INCLUSIONS|PREMIUM TRAVEL|\Z)",
            text,
            re.DOTALL | re.IGNORECASE,
        )
    if not section_m:
    ...
    pattern = re.compile(
        r"Option\s*0?(\d+)\s*:\s*(?:(Deluxe|Premium|Luxury|Ultra\s*Luxury)\s+)?(.+?)(?=Option\s*0?\d+\s*:|$)",
        re.DOTALL | re.IGNORECASE,
    )
...
def build_meta(serial: str, text: str, raw: str) -> dict:
    title = extract_title_sg(text, serial)
    tour_code = extract_tour_code(normalize_sg_raw(raw))
    destinations = extract_destinations_sg(text)
    hotels = parse_sg_hotel_options(text)
    if len(hotels) < 4:
        alt = parse_hotel_options(text)
        if len(alt) > len(hotels):
            hotels = alt
    nights = _compact_nights_label(_jh._infer_nights(text))
    primary_loc = destinations.split("•")[0].strip() or "Singapore"
    if 0 < len(hotels) < 4:
        ...
        hotels = [
            {
                ...
                "category": cat,
                "name": f"TRAGUIN Handpicked {cat} Property",
                "location": destinations.split("•")[0].strip() or "Singapore",
                "nights_label": nights,
                "room_type": "Deluxe Room",
                "meal_plan": "MAPAI (Breakfast & Dinner)",
                "stars": 5 if "ultra" in cat.lower() else 4,
    ...
    included, excluded = parse_sg_inclusions_exclusions(raw)
    if len(included) < 3:
        ...
        if len(short_inc) >= 3:
            ...
            included, excluded = _default_sg_inclusions()
    return {
        "serial": serial,
        "tour_code": tour_code,
...
def build_package_highlights(meta: dict, text: str) -> list[str]:
    ...
    category = extract_category_sg(text)
    destinations = meta.get("destinations") or extract_destinations_sg(text)
    highlights = [
        f"Serial code {serial} | TRAGUIN tour code {tour_code}",
        f"Country: Singapore, Asia | Category: {category}",
        ...
        f"Vehicle / Meals: {extract_vehicle_meals_sg(text)}",
    ]
...
def emit_builder(serial: str, meta: dict, text: str, raw_text: str) -> str:
    ...
    tour_code = meta["tour_code"]
    moods = meta["moods"]
    days = _clean_day_descriptions(parse_day_blocks_sg(text))
    hotels = meta["hotels"]
    included = _clean_items(meta.get("included") or parse_sg_inclusions_exclusions(raw_text)[0])
    excluded = meta.get("excluded")
    if excluded is None:
        _, excluded = parse_sg_inclusions_exclusions(raw_text)
...
{acts},
    ...
    destinations = meta.get("destinations") or extract_destinations_sg(text)
    seo_desc = (
        f"Premium {duration} Singapore package ({serial} / {tour_code}): "
...
def main() -> None:
    header = '''"""Builder functions for SG-001 through SG-015 Singapore international packages."""
...
'''
    ...
    builder_names = [f"build_sg_{serial.split('-')[1]}" for serial in BUILDER_ORDER if (PDF_DIR / f"{serial}.txt").exists()]
