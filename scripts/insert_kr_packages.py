#!/usr/bin/env python3
"""Insert KR-001 through KR-003 South Korea packages (no images)."""
...
from kr_batch_package_defs import KR_BUILDERS
...
def get_or_create_south_korea_destination_id() -> str:
    with SessionLocal() as db:
        ...
        if destination is None:
            destination = Destination(
                ...
                india_region=None,
                description=(
                    "Seoul, Jeju Island, Busan, and Gyeongju — ancient palaces, K-culture, volcanic landscapes, "
                    "coastal luxury, and world-class hospitality for families, couples, and discerning travelers."
                ),
                starting_price=0,
                moods=["family", "luxury", "romantic", "cultural", "nature", "beach"],
                is_published=True,
            )
            db.add(destination)
            db.flush()
...
def main() -> None:
    dest_id = get_or_create_south_korea_destination_id()
    with SessionLocal() as db:
        for builder in KR_BUILDERS:
